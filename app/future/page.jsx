...
const ScannerLine = () => (
  <div className="absolute inset-0 pointer-events-none overflow-hidden rounded-xl">
    <div className="w-full h-[2px] bg-gradient-to-r from-transparent via-violet-400/50 to-transparent absolute top-0 -translate-y-full animate-scan-fast" />
  </div>
);
  ...
  return (
    <PageTransitionWrapper>
      <main className="relative min-h-screen bg-[#050308] overflow-hidden selection:bg-violet-500/30">
        ...
        <div className="absolute inset-0 bg-[linear-gradient(rgba(139,92,246,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(139,92,246,0.03)_1px,transparent_1px)] bg-[size:40px_40px] [mask-image:radial-gradient(ellipse_at_center,black_40%,transparent_100%)] opacity-50" />
        
        {/* Ambient Glow */}
        <div className="absolute bottom-0 right-0 w-[600px] h-[600px] bg-violet-600/10 blur-[120px] rounded-full pointer-events-none mix-blend-screen" />
        ...
        <div className="relative z-10 px-4 py-8 md:py-16">
          ...
          <header className="max-w-7xl mx-auto mb-24 relative text-center md:text-left">
            <div className="md:pl-10 relative">
              <div className="absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b from-transparent via-violet-500/50 to-transparent hidden md:block" />
              ...
              <div className="inline-flex items-center gap-2 px-3 py-1 mb-6 rounded border border-violet-500/30 bg-violet-500/5 backdrop-blur-sm">
                <span className="text-[10px] font-mono text-violet-300 tracking-widest uppercase">Target: Future_Horizon</span>
              </div>
              
              <h1 className="text-5xl md:text-8xl font-bold tracking-tighter mb-6 text-transparent bg-clip-text bg-gradient-to-r from-violet-100 via-violet-400 to-fuchsia-500 font-sans">
              ...
              <p className="text-lg text-violet-100/60 max-w-2xl font-light tracking-wide md:border-l-2 md:border-transparent md:pl-0">
                Initializing predictive models. The following data represents our calculated trajectory towards becoming a global nexus of innovation.
              </p>
            </div>
          </header>
          ...
          <section className="max-w-7xl mx-auto mb-32">
             <div className="flex items-center gap-4 mb-12">
               <h2 className="text-sm font-mono text-violet-400 tracking-[0.3em] uppercase">Temporal Roadmap</h2>
               <div className="h-px flex-1 bg-gradient-to-r from-violet-500/30 to-transparent" />
             </div>

            <div className="relative">
              {/* Connection Line */}
              <div className="absolute top-[2.5rem] left-0 right-0 h-0.5 bg-violet-900/50 hidden md:block">
                  ...
                  <div key={index} className="relative group">
                    ...
                    <div className="h-full bg-violet-900/5 border border-violet-500/10 hover:border-violet-500/40 p-6 backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 relative overflow-hidden group-hover:bg-violet-900/10">
                      ...
                      <div className="flex justify-between items-start mb-4">
                        <span className="text-4xl font-bold text-violet-500/20 group-hover:text-violet-400/80 transition-colors font-mono">{item.year}</span>
                        <span className={`text-[10px] font-mono px-2 py-0.5 rounded border ${
                            index === 3 ? 'border-red-500/30 text-red-400' : 'border-violet-500/30 text-violet-400'
                        }`}>
                            {item.status}
                        </span>
                      </div>
                      ...
                      <p className="text-sm text-violet-200/50 leading-relaxed">{item.description}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <section className="max-w-7xl mx-auto mb-32">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {futureTechnologies.map((tech, index) => (
                <div key={index} className="group relative bg-black/40 border border-violet-500/20 hover:border-violet-400/60 transition-all duration-300 overflow-hidden">
                  <ScannerLine />
                  ...
                  <div className="p-6 relative z-10">
                    <div className="flex justify-between items-start mb-6">
                       ...
                       <div className="text-[10px] font-mono text-violet-600 group-hover:text-violet-400">{tech.code}</div>
                    </div>

                    <h3 className="text-lg font-bold text-white mb-2 group-hover:text-violet-300 transition-colors">{tech.title}</h3>
                    <p className="text-xs text-violet-200/50 mb-6 h-10">{tech.description}</p>
          ...
          <section className="max-w-7xl mx-auto mb-20">
            <div className="text-center mb-16">
               <h2 className="text-3xl font-bold text-white mb-2 tracking-tight">Strategic Alliances</h2>
               <div className="w-24 h-1 bg-gradient-to-r from-transparent via-violet-500 to-transparent mx-auto" />
            </div>
            ...
            <div className="mb-12">
               <div className="flex items-center justify-center gap-4 mb-6">
                  ...
                  <span className="text-xs font-mono text-violet-300 tracking-[0.2em]">TIER 1 : PLATINUM</span>
                  <div className="h-px w-12 bg-violet-500/30" />
               </div>
               
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
                 {sponsors.filter(s => s.tier === "Platinum").map((sponsor, i) => (
                    <div key={i} className="group relative h-32 bg-violet-950/10 border border-violet-500/20 flex items-center justify-center overflow-hidden hover:bg-violet-500/5 transition-colors">
                       ...
                       <span className="text-2xl font-bold text-white tracking-widest group-hover:scale-110 transition-transform duration-300 font-sans z-10">{sponsor.name}</span>
                    </div>
                 ))}
               </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 max-w-5xl mx-auto">
               ...
               <div>
                  <div className="text-center mb-6">
                     <span className="text-[10px] font-mono text-amber-400/80 tracking-[0.2em] border border-amber-500/20 px-2 py-1 bg-amber-500/5">TIER 2 : GOLD</span>
                  </div>
                  <div className="space-y-3">
                     {sponsors.filter(s => s.tier === "Gold").map((sponsor, i) => (
                        <div key={i} className="py-4 border-b border-amber-500/20 text-center hover:bg-amber-500/5 transition-colors cursor-default">
                           <span className="text-amber-100/80 font-mono text-sm">{sponsor.name}</span>
                        </div>
                     ))}
                  </div>
               </div>
               ...
               <div>
                  <div className="text-center mb-6">
                     <span className="text-[10px] font-mono text-slate-400/80 tracking-[0.2em] border border-slate-500/20 px-2 py-1 bg-slate-500/5">TIER 3 : SILVER</span>
                  </div>
                  <div className="flex flex-wrap justify-center gap-3">
                     {sponsors.filter(s => s.tier === "Silver").map((sponsor, i) => (
                        <div key={i} className="px-4 py-2 border border-slate-500/20 rounded-full text-xs text-slate-400 font-mono hover:border-slate-400 hover:text-white transition-colors">
                           {sponsor.name}
                        </div>
                     ))}
                  </div>
          ...
          <section className="max-w-4xl mx-auto mt-32">
             <div className="relative border border-violet-500/30 bg-black/60 overflow-hidden rounded-2xl p-8 md:p-12 text-center">
                ...
                <h2 className="text-3xl md:text-5xl font-bold text-white mb-6">Initialize Partnership Protocol</h2>
                <p className="text-violet-200/60 mb-8 max-w-xl mx-auto text-lg">
                   Join the trajectory. Contribute to the future of innovation and secure your coordinates in the next era.
                </p>
                
                <div className="flex flex-col md:flex-row justify-center gap-6">
                   <button className="px-8 py-4 bg-violet-600 hover:bg-violet-500 text-white font-bold tracking-widest uppercase text-xs transition-all hover:shadow-[0_0_20px_rgba(139,92,246,0.5)] clip-path-polygon" style={{ clipPath: 'polygon(10px 0, 100% 0, 100% calc(100% - 10px), calc(100% - 10px) 100%, 0 100%, 0 10px)' }}>
                      Become a Partner
                   </button>
