...
const EnergyChart = () => {
  ...
  const [data, setData] = useState(
    Array.from({ length: points }, () => Math.random() * 40 + 30),
  );
  ...
  const pathD =
    `M 0 ${height} ` +
    data
      .map((val, i) => `L ${(i / (points - 1)) * width} ${height - val}`)
      .join(" ") +
    ` L ${width} ${height} Z`;
  const lineD = data
    .map(
      (val, i) =>
        `${i === 0 ? "M" : "L"} ${(i / (points - 1)) * width} ${height - val}`,
    )
    .join(" ");
  ...
  return (
    <div className="w-full h-32 bg-[#051505] border border-emerald-500/30 rounded-lg overflow-hidden relative">
      ...
      <div className="absolute top-2 left-2 text-[8px] font-mono text-emerald-500">
        REAL-TIME ENERGY OUTPUT
      </div>
      <div className="absolute bottom-2 right-2 text-[8px] font-mono text-emerald-400 animate-pulse">
        LIVE
      </div>
    </div>
  );
    ...
    <PageTransitionWrapper>
      ...
      >
        ...
        <div className="relative z-10 container mx-auto px-4 py-20 min-h-screen flex flex-col items-center justify-center text-center">
          ...
          >
            <span
              className={`px-4 py-1 rounded-full text-[10px] font-bold tracking-widest uppercase transition-colors ${isCycled ? "text-emerald-500" : "bg-emerald-500/10 text-emerald-300"}`}
            >
          ...
          <p className="max-w-2xl mx-auto text-emerald-100/70 text-lg mb-12 leading-relaxed">
            A net-zero ecosystem powered by renewable energy and integrated with
            nature. Where technology meets biology to create a regenerative
            future for Ingenium.
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 w-full max-w-6xl items-center">
            ...
            <div className="bg-[#0a1a0a]/80 border border-emerald-500/20 p-6 rounded-2xl backdrop-blur-sm group hover:border-emerald-400/50 transition-all">
              ...
              <div className="text-xs text-emerald-400/60 uppercase tracking-wider">
                Renewable Energy
              </div>
            </div>
