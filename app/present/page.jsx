...
const OpsCorner = ({ className }) => (
  <svg className={`absolute w-4 h-4 text-blue-500 ${className}`} viewBox="0 0 24 24" fill="none">
...
const AudioWave = () => (
  <div className="flex gap-0.5 items-end h-4">
    {[...Array(6)].map((_, i) => (
      <div key={i} className="w-1 bg-blue-400 animate-music-bar" style={{ animationDelay: `${i * 0.1}s` }} />
    ))}
  </div>
);
      ...
      <main className="relative min-h-screen bg-[#030712] overflow-hidden selection:bg-blue-500/30">
        ...
        <div className="relative z-10 px-4 py-6 md:py-10 flex flex-col min-h-screen">
            ...
            <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-4">
              ...
              <div className="flex items-center gap-6">
                <Link href="/?state=navigation">
                  <button className="flex items-center gap-2 text-blue-400/60 hover:text-white transition-colors group">
                    ...
                    <span className="text-[10px] font-mono tracking-widest uppercase">Ops Center</span>
                  </button>
                </Link>
              ...
              <div className="flex items-center gap-6 text-right">
                <div className="hidden md:block">
                  <div className="text-[9px] text-blue-500/50 uppercase tracking-wider font-mono">System Status</div>
                ...
                <div className="bg-blue-950/30 border border-blue-500/30 px-4 py-2 rounded">
                  <div className="text-[9px] text-blue-400/60 uppercase tracking-wider font-mono text-center">Local Time</div>
          ...
          <div className="max-w-7xl mx-auto w-full mb-10">
            <div className="flex flex-wrap gap-2 p-1 bg-black/40 border border-blue-500/20 rounded-lg backdrop-blur-md inline-flex">
               {['dashboard', 'schedule', 'access'].map((tab) => (
                 ...
                 >
                   {activeTab === tab && <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent animate-scan-fast" />}
                   {tab === 'access' ? 'Registrations' : tab.charAt(0).toUpperCase() + tab.slice(1)}
                 </button>
               ))}
            </div>
          </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-fade-in-up">
                ...
                <div className="lg:col-span-8 space-y-6">
                   <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-[10px] bg-blue-500/10 text-blue-300 px-2 py-0.5 rounded border border-blue-500/20">2 SIGNALS FOUND</span>
                   </div>

                   {/* Live Cards */}
                   {ongoingEvents.map((event, i) => (
                     <div key={i} className="group relative bg-[#050a14] border border-blue-500/30 rounded-lg overflow-hidden hover:border-blue-400 transition-all duration-300">
                        {/* Monitor Scan Line */}
                        <div className="absolute inset-0 bg-[linear-gradient(transparent_50%,rgba(0,0,0,0.4)_50%)] bg-[length:100%_4px] opacity-20 pointer-events-none z-10" />
                        <div className="absolute inset-0 bg-gradient-to-r from-blue-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity z-0" />
                        ...
                        <div className="p-6 relative z-20 flex flex-col md:flex-row gap-6">
                           ...
                           <div className="w-full md:w-48 h-32 bg-blue-900/20 rounded border border-blue-500/20 flex items-center justify-center relative overflow-hidden">
                              ...
                              <div className="absolute top-2 left-2"><LiveIndicator /></div>
                           </div>

                           {/* Info */}
                           <div className="flex-1">
                              <div className="flex justify-between items-start mb-2">
                                 <h3 className="text-xl font-bold text-white group-hover:text-blue-300 transition-colors">{event.name}</h3>
                                 <AudioWave />
                              </div>
                              
                              <div className="flex flex-wrap gap-4 text-xs font-mono text-blue-400/60 mb-4">
                                 <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 bg-blue-500 rounded-full"/> {event.venue}</span>
                                 <span className="flex items-center gap-1"><svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg> {event.viewers}</span>
                              </div>
                              ...
                              <div className="w-full h-1 bg-blue-900/30 rounded-full overflow-hidden flex items-center">
                                 <div className="h-full bg-blue-500" style={{ width: event.bandwidth }} />
                              </div>
                              <div className="text-[9px] text-right text-blue-500/40 font-mono mt-1">STREAM STABILITY: {event.bandwidth}</div>
                           </div>
                        </div>
                     </div>
                   ))}
                ...
                <div className="lg:col-span-4 space-y-6">
                   <div className="bg-[#050a14] border border-blue-500/20 rounded-lg p-5">
                      <h3 className="text-xs font-bold text-white uppercase tracking-widest mb-4 border-b border-blue-500/20 pb-2">Upcoming Transmissions</h3>
                      <div className="space-y-4">
                         {upcomingEvents.map((ev, i) => (
                            <div key={i} className="flex items-center gap-3 group cursor-pointer hover:bg-blue-500/5 p-2 rounded transition-colors">
                               <div className={`w-1 h-8 rounded-full ${ev.priority === 'MAX' ? 'bg-red-500' : ev.priority === 'HIGH' ? 'bg-orange-500' : 'bg-blue-500'} group-hover:h-10 transition-all`} />
                               <div className="flex-1">
                                  <div className="text-sm font-bold text-gray-200 group-hover:text-blue-300">{ev.name}</div>
                                  <div className="text-[10px] font-mono text-blue-500/50">{ev.time} | {ev.venue}</div>
                               </div>
                               <button className="text-blue-500 opacity-0 group-hover:opacity-100 transition-opacity text-[10px] border border-blue-500 px-2 py-1 rounded hover:bg-blue-500 hover:text-white">NOTIFY</button>
                            </div>
                         ))}
                      </div>
                   </div>
                   ...
                   <div className="bg-black border border-blue-500/30 rounded-lg p-4 font-mono text-[10px] h-40 overflow-hidden relative opacity-70">
                      ...
                      <div className="space-y-1 text-blue-400/80">
                         ...
                         <p className="text-green-400"> connection established (12ms)</p>
                         <p> fetching event_manifest.json</p>
                         <p> loading assets...</p>
                         <p className="animate-pulse"> monitoring system health...</p>
                      </div>
                   </div>
                </div>
              </div>
            ...
            {activeTab === 'schedule' && (
               <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-fade-in-up">
                  {scheduleData.map((day, i) => (
                     <div key={i} className={`relative flex flex-col h-full rounded-xl border ${day.active ? 'border-blue-400 bg-blue-900/10' : 'border-blue-500/20 bg-[#050a14]/50'} overflow-hidden transition-all duration-300 hover:border-blue-500/50`}>
                        {day.active && <div className="absolute top-0 inset-x-0 h-0.5 bg-blue-400 shadow-[0_0_10px_#3b82f6]" />}
                        
                        {/* Header */}
                        <div className="p-4 border-b border-blue-500/10 bg-black/20">
                           <div className="flex justify-between items-center">
                              <h3 className={`text-lg font-bold ${day.active ? 'text-white' : 'text-gray-400'}`}>{day.day}</h3>
                              <span className="text-xs font-mono text-blue-500/60">{day.date}</span>
                           </div>
                        </div>
                        ...
                        <div className="p-4 space-y-4 flex-1 relative">
                           {day.active && (
                              ...
                              <div className="absolute left-0 right-0 top-1/3 h-[1px] bg-red-500 z-10 flex items-center">
                                 <div className="w-full h-[1px] bg-red-500 animate-pulse shadow-[0_0_8px_red]" />
                                 <span className="absolute right-2 text-[8px] bg-red-500 text-black font-bold px-1 rounded-sm">NOW</span>
                              </div>
                           )}

                           {day.events.map((ev, j) => (
                              <div key={j} className="flex gap-3 relative z-0 opacity-90 hover:opacity-100 transition-opacity">
                                 <div className="w-12 text-[10px] font-mono text-blue-400 pt-1 text-right">{ev.time}</div>
                                 <div className="relative flex-1 pb-4 border-l border-blue-500/10 pl-4">
                                    <div className={`absolute -left-[5px] top-1.5 w-2.5 h-2.5 rounded-full border-2 ${ev.status === 'Live' ? 'bg-red-500 border-red-500 animate-pulse' : ev.status === 'Done' ? 'bg-blue-900 border-blue-700' : 'bg-black border-blue-500/30'}`} />
                                    <h4 className={`text-sm font-medium ${ev.status === 'Live' ? 'text-white' : 'text-gray-400'}`}>{ev.name}</h4>
                                    <p className="text-[10px] text-blue-500/50">{ev.venue}</p>
                                 </div>
                              </div>
                           ))}
                        </div>
            ...
            {activeTab === 'access' && (
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in-up">
                  {registrations.map((reg, i) => {
                     ...
                     return (
                        <div key={i} className="group relative bg-[#080c14] border border-blue-500/20 rounded-xl overflow-hidden hover:bg-blue-900/5 transition-all duration-300">
                              ...
                              <div>
                                 <div className="flex justify-between items-start mb-4">
                                    <div className="p-2 bg-blue-500/10 rounded-lg border border-blue-500/20 text-blue-400">
                                       <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0c0 .883-.393 1.627-1.08 1.998" /></svg>
                                    </div>
                                    <span className={`text-[10px] font-bold px-2 py-1 rounded border uppercase ${isFull ? 'border-red-500/30 text-red-500 bg-red-500/5' : 'border-green-500/30 text-green-500 bg-green-500/5'}`}>
                                       {reg.status}
                                    </span>
                                 </div>
                                 <h3 className="text-xl font-bold text-white mb-1 group-hover:text-blue-300 transition-colors">{reg.name}</h3>
                                 <p className="text-xs text-blue-500/50 font-mono">ACCESS_ID: REG-{1000+i}</p>
                              </div>

                              <div className="mt-6">
                                 <div className="flex justify-between text-[10px] font-mono text-gray-400 mb-1">
                                    <span>CAPACITY</span>
                                    <span className={isFull ? 'text-red-500' : 'text-blue-400'}>{reg.spots === 999 ? 'UNLIMITED' : `${reg.spots} / ${reg.total}`}</span>
                                 </div>
                                 ...
                                 {reg.spots !== 999 && (
                                    <div className="h-1.5 w-full bg-gray-800 rounded-full overflow-hidden">
                                       <div 
                                          ...
                                          style={{ width: `${(reg.spots / reg.total) * 100}%` }} 
                                       />
                                    </div>
                                 )}

                                 <button disabled={isFull} className={`w-full mt-4 py-3 text-xs font-bold uppercase tracking-widest transition-all ${isFull ? 'bg-gray-800 text-gray-500 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-500 text-white shadow-lg hover:shadow-blue-500/50'}`}>
                                    {isFull ? 'Access Denied' : 'Initialize Registration'}
                                 </button>
                              </div>
                           </div>
                        </div>
