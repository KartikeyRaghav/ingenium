...
function RegistrationContent() {
  ...
  return (
    <div className="relative z-10 w-full max-w-md mx-auto">
      ...
      <div className={`relative bg-[#050a14]/80 backdrop-blur-xl border border-blue-500/30 rounded-xl p-8 overflow-hidden transition-all duration-300 ${status === 'transmitting' ? 'border-blue-400 shadow-[0_0_30px_rgba(59,130,246,0.3)]' : ''}`}>
        ...
        {status === "complete" && (
          <div className="absolute inset-0 z-50 bg-[#050a14]/95 flex flex-col items-center justify-center text-center p-6 animate-fade-in-up">
            ...
            <h2 className={`text-2xl font-bold text-white mb-2 ${glitch ? 'animate-flicker' : ''}`}>TRANSMISSION COMPLETE</h2>
            <p className="text-blue-400/60 text-sm font-mono mb-8">
              Your data has been encrypted and stored in the Chronoverse archive.
            </p>
            <button 
        ...
        {/* The Form */}
        <form onSubmit={handleSubmit} className={`space-y-6 ${status !== 'idle' ? 'opacity-50 pointer-events-none' : ''}`}>
          
          {/* Input Group */}
          {[
            { id: "fullName", label: "Identity Designation", placeholder: "Enter Full Name", type: "text" },
            { id: "email", label: "Comms Frequency", placeholder: "Enter Email Address", type: "email" },
            { id: "studentId", label: "Access ID", placeholder: "Student / ID Number", type: "text" },
            { id: "department", label: "Sector / Department", placeholder: "e.g., Computer Science", type: "text" },
          ].map((field) => (
            <div key={field.id} className="relative group">
              ...
              />
              {/* Corner accents */}
              <div className="absolute bottom-0 left-0 w-0 h-[1px] bg-blue-400 group-focus-within:w-full transition-all duration-500" />
            </div>
          ))}
        ...
        <div className="fixed top-6 left-6 z-20">
           <Link href="/present">
            <button className="flex items-center gap-2 text-blue-400/60 hover:text-white transition-colors group">
                ...
                <span className="text-[10px] font-mono tracking-widest uppercase hidden md:block">Abort Sequence</span>
            </button>
           </Link>
        </div>

        <Suspense fallback={
          <div className="text-blue-500 font-mono text-xs animate-pulse">LOADING MODULE...</div>
        }>
          <RegistrationContent />
        </Suspense>
