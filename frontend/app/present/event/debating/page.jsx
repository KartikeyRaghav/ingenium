            ...
            <div className="p-6 bg-black/40 border border-white/5 rounded-2xl">
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {[
                  ...
                  <span
                    key={tag}
                    className="text-[11px] bg-white/5 px-2 py-1 rounded border border-white/10 text-blue-300 uppercase"
                  >
                    {tag}
                  </span>
                ))}
...
function EvaluationMetric({ label }) {
  return (
    <div className="flex items-center gap-2 text-[11px] text-white/40 uppercase font-bold group">
...
function BountyNode({ rank, amount, label }) {
  return (
    <div className="text-center group">
      <div className="text-[10px] text-blue-500 uppercase mb-1 font-bold tracking-tighter">
        {rank}_TIER
      </div>
      <div className="text-xl font-black text-white italic group-hover:scale-110 transition-transform">
        ₹{amount}
      </div>
      <div className="text-[9px] text-blue-400/40 uppercase mt-1 font-bold">
        {label}
      </div>
    </div>
  );
